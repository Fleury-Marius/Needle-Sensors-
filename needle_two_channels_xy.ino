...
// Your sensors measure horizontal axes (X and Y). If the syringe tilts,
// the world beam picks up a Z component and the correction starts leaking
// into vertical - which is why a blocked sensor could make the needle
// jump up. Zeroing the Z component of the beam confines each sensor's
// correction to the horizontal plane it is actually meant to observe.
// Set to 0 if you ever mount a sensor to genuinely measure vertical.
#define RANGE_HORIZONTAL_ONLY 1
...
// Two beams within this |cos| of each other are treated as the same axis
// and averaged instead of applied twice. Perpendicular beams (dot ~ 0)
// are always well under this, so they run as two independent channels.
#define BEAM_PARALLEL_DOT 0.99f
...
RangeChannel ch1 = {BEAM1_BODY_X, BEAM1_BODY_Y, BEAM1_BODY_Z,
                    false, 0.0f, -1.0f, -1.0f, 0.0f, 0, 0, 0};
...
{
#if RANGE_HORIZONTAL_ONLY
    // Drop the vertical component and renormalise, so this sensor only
    // ever corrects in the horizontal plane. Guard against a near-vertical
    // beam (nothing horizontal left to normalise) by bailing out.
    uz = 0.0f;
    float hmag = sqrt(ux*ux + uy*uy);
    if (hmag < 0.3f) { ch->state = 0; return; }  // beam too vertical to use
    ux /= hmag; uy /= hmag;
#endif
...
void applyRangeCorrection(bool is_stationary)
{
    ...
    // current fused orientation. This is what keeps the correction landing
    // on the correct world axis even as the syringe tilts and turns.
    ...
    // If the two beams happen to point almost the same way, correcting
    // both would double-count one axis. Average them instead.
    bool parallel = fabs(u1x*u2x + u1y*u2y + u1z*u2z) > BEAM_PARALLEL_DOT;
    ...
    if (parallel) {
        ...
        else { resetChannel(&ch1); ch2.state = 0; return; }

        updateRangeChannel(&ch1, u1x, u1y, u1z, d, is_stationary);
        ch2.state = ch1.state;
        ch2.innov = ch1.innov;
        return;
    }

    // The normal perpendicular case: two independent axes, one per sensor.
...
void loop()
{
    ...
    if (++extras_group > 4) extras_group = 0;
